			...
			<>
				<div className={styles.Container}>
					<form action='/api/perfil' method='PATCH' className={styles.Form}>
						...
						<div className={styles.InputBox}>
							<label className={styles.Label}>Email</label>
							<input disabled type='text' name='email' className={styles.Input} value='Loading...' onChange={() => { }} ></input>
						</div>
						<br></br>
						<div className={styles.InputBox}>
							<span></span>
							<label>Nome Completo</label>
							<input disabled type="text" name='name' placeholder={"Nome Completo"} className={styles.Input} value={name} onChange={e => { setName(e.target.value) }} required></input>
						</div>
						<div className={styles.InputBox}>
							<label>Data de Nascimento</label>
							<input disabled type="date" name='birthDate' placeholder={"Data de Nascimento"} className={styles.Input} value={birthDate} onChange={e => { setBirthDate(e.target.value) }} required></input>
						</div>
						<div className={styles.InputBox}>
							<label>CPF</label>
							<input disabled type="text" name='cpf' placeholder={"111.222.333-99"} value={cpf} className={styles.Input} onChange={e => { setCPF(e.target.value) }} required></input>
						</div>
						<div className={styles.InputBox}>
							<label>Telefone | Celular</label>
							<input disabled type="tel" name='phone' placeholder={"(11) 91234-5678"} value={phone} className={styles.Input} onChange={e => { setPhone(e.target.value) }}></input>
						</div>
						<button type="submit" className={styles.SubmitButton}>Salvar</button>
					</form>
				</div >
